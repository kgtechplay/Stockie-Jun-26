...
def find_prediction_files(underlying: str) -> list:
    ...
    This expects files that include both prediction and option-selection
    columns. Plain historical prediction files are handled by
    ...
    # Filter to only include files that already have option-selection output.
    filenames = []
    for f in files:
        filename = os.path.basename(f)
        try:
            cols = pd.read_csv(f, nrows=0).columns
            if "option_instrument_token" in cols or "option_tradingsymbol" in cols:
