...
from src.technical_analysis.prediction.schema import RegimeSnapshot, StrategySignal, UnderlyingFeatureSnapshot
...
def bullish_features() -> UnderlyingFeatureSnapshot:
    return UnderlyingFeatureSnapshot(
        ...
        volume_avg_20d=100000,
        volume_ratio=1.5,
        trend_efficiency=0.55,
        range_position=0.75,
        relative_strength_vs_sector=0.03,
        relative_strength_vs_benchmark=0.04,
    )


def bearish_features() -> UnderlyingFeatureSnapshot:
    return UnderlyingFeatureSnapshot(
        symbol="TEST",
        trade_date="2026-05-15",
        close=90,
...
def regime(stock: str, sector: str | None = None, benchmark: str | None = None) -> RegimeSnapshot:
...
class UnderlyingPredictionViewTests(unittest.TestCase):
    ...
    def test_bearish_trend_scores_high(self) -> None:
        score = score_underlying_view(
            bearish_features(),
            regime("TREND_DOWN", "TREND_DOWN", "TREND_DOWN"),
            "BEARISH",
    ...
    def test_choppy_regime_blocks_option_eligibility(self) -> None:
        ...
        reg = regime("CHOPPY", "TREND_UP", "TREND_UP")
    ...
    def test_underlying_view_maps_high_score_to_strong_bias(self) -> None:
        features = bullish_features()
        reg = regime("TREND_UP", "TREND_UP", "TREND_UP")
        signals = build_strategy_signals({"MaTrend_001": "CALL", "trendUpRangeBreakout": "CALL"}, features, reg)
        view = build_underlying_view("TEST", "2026-05-15", features, reg, signals)
    ...
    def test_conflicting_strategies_return_no_position(self) -> None:
        ...
        signals = [
            ...
            StrategySignal(
                ...
                raw_signal="PUT",
                direction="BEARISH",
                setup_type="TREND_DOWN_RALLY_SHORT",
                score=80,
                confidence="HIGH",
                expected_holding_days=3,
                expected_move_pct=0.02,
