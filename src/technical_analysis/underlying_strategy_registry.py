...
UnderlyingPredictionFunction = Callable[[object], str]


def _load_underlying_modules() -> list[ModuleType]:
    this_dir = Path(__file__).parent
    modules: list[ModuleType] = []
    for path in sorted(this_dir.glob("underlying_prediction_*.py")):
        if path.name in {"underlying_prediction_common.py", "underlying_registry.py"}:
            continue
...
def load_underlying_prediction_strategies() -> dict[str, UnderlyingPredictionFunction]:
    ...
    for module in _load_underlying_modules():
