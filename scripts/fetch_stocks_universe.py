...
"""
Build stocks_universe.csv by fetching from two independent sources and merging.
...
Pipeline:
  ...
  Step 2 — Yahoo Finance  (runs by default; skip with --no-yfinance)
...
Usage:
    python scripts/fetch_stocks_universe.py              # NSE + yfinance
    ...
    python scripts/fetch_stocks_universe.py --yf-delay 0.6   # slower = safer
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build stocks_universe.csv from NSE + yfinance, merged"
    )
    parser.add_argument(
        "--fo-only",
        action="store_true",
    ...
    parser.add_argument(
        "--no-yfinance",
        action="store_true",
        help="Skip Yahoo Finance step (faster; small-cap sectors will be blank)",
    )
    parser.add_argument(
        "--yf-delay",
        type=float,
