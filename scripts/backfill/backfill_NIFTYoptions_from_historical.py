...
from scripts.daily.calculate_option_snapshot_calc import Settings as CalcSettings
from scripts.daily.calculate_option_snapshot_calc import calculate_snapshot_ids
...
def load_all_option_instruments_for_snapshot(
    ...
    Optional filters are provided only to make testing safer:
      ...
      --max-instruments
    """
    table = safe_table_name(settings.option_instrument_table)
    ...
    for row in rows:
        if row.instrument_token is None:
            continue
...
def run_backfill_all_options_from_historical(
    ...
    db = DatabaseClient(app_settings)
    db.connect()
...
def main() -> None:
    ...
    parser.add_argument("--from-date", "--start", dest="from_date", required=True, help="YYYY-MM-DD")
    parser.add_argument("--to-date", "--end", dest="to_date", required=True, help="YYYY-MM-DD")
    ...
    run_backfill_all_options_from_historical(
        global_start=date.fromisoformat(args.from_date),
        global_end=date.fromisoformat(args.to_date),
