# scripts/backfill/backfill_nifty_options.py
"""
Backfill dbo.OptionSnapshot + dbo.OptionSnapshotCalc for INDEX option instruments.

For each row in dbo.OptionInstrument (underlying IN NIFTY, BANKNIFTY):
  ...
  - Underlying price uses index day candle (open/close)
  - Uses Kite interval="day" (one API call per option covers the full range)

Usage:
    python scripts/backfill/backfill_nifty_options.py
    python scripts/backfill/backfill_nifty_options.py --start 2025-01-01 --end 2026-04-30
    python scripts/backfill/backfill_nifty_options.py --underlying NIFTY
"""
...
def _resolve_index_tokens(
    ...
    if need_lookup:
        ...
        for ki in nse_instr:
            ...
            canonical = _KITE_TO_CANONICAL.get(ts, ts)
...
def _fetch_underlying_day_candles(
    ...
    """Fetch day-interval candles for the index. Returns {trade_date: candle}."""
    try:
        ...
        print(f"[WARN] Failed fetching underlying day candles (token={token}): {e}")
        return {}
...
def run_backfill_options(
    ...
    # Load INDEX watched instruments
    ...
    print(f"Index underlyings: {', '.join(target_underlyings)}")

    kite_client = KiteClient(settings)
    kite_client.authenticate()
    index_tokens = _resolve_index_tokens(kite_client, watched)
    ...
    all_db_ids = [db_id for db_id in db_id_to_fetch_date]
    ...
    # Phase 2: fetch underlying day candles (one call per index, full range)
    ...
    for underlying, token in index_tokens.items():
        print(f"Fetching {underlying} day candles ({overall_start} to {eff_end})...")
        underlying_day_candles[underlying] = _fetch_underlying_day_candles(
            kite_client, token, overall_start, eff_end
        )
        print(f"  {underlying}: {len(underlying_day_candles[underlying])} trading days")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Backfill OptionSnapshot (2/day at 09:15 + 15:15) for INDEX instruments"
    )
    parser.add_argument(
        "--start", default=None,
    ...
    parser.add_argument(
        ...
        help="Comma-separated underlyings to restrict (e.g. NIFTY,BANKNIFTY)",
    )
