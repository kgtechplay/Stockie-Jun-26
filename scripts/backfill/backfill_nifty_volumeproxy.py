...
# Main
# -----------------------
_SUPPORTED_VOLUMEPROXY = {"NIFTY", "BANKNIFTY"}
...
def run_backfill_volumeproxy(
    ...
    # Load from WatchedInstrument when not explicitly supplied
    if underlyings:
        target_underlyings = [u.upper() for u in underlyings]
    else:
        try:
            target_underlyings = _underlyings_from_watched()
        except Exception as e:
            print(f"[WARN] Could not load underlyings from WatchedInstrument ({e}); falling back to defaults.")
            target_underlyings = list(UNDERLYINGS)

    # NSE FO bhavcopy only carries NIFTY/BANKNIFTY as index futures
    unsupported = [u for u in target_underlyings if u not in _SUPPORTED_VOLUMEPROXY]
    if unsupported:
        print(f"[WARN] NSE FO bhavcopy only supports {_SUPPORTED_VOLUMEPROXY}. Skipping: {unsupported}")
    target_underlyings = [u for u in target_underlyings if u in _SUPPORTED_VOLUMEPROXY]
    if not target_underlyings:
        print("No supported underlyings to process.")
        return {
            "underlyings": [],
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Backfill MarketActivityDaily from NSE FO bhavcopy for WatchedInstrument INDEX entries")
